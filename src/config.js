 ...
 * v5.10.5 - No login required, X-Admin-Token header fix on protected endpoints
 */
...
function App() {
  ...
  const generateComprehensiveSummary = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/orders/${selectedOrder.order_id}/comprehensive-summary`,
        {
          method: 'POST',
          headers: {
            'X-Admin-Token': ADMIN_TOKEN
      ...
      if (data.summary) {
        ...
        setComprehensiveSummary('No summary returned.')
      }
    } catch (err) {
  ...
  return (
    <div className="app">
          ...
          filteredOrders.map((order) => (
            <OrderCard
              ...
              onOpenShippingManager={(shipment) =>
                openShippingManager(shipment, order)
              }
              onUpdate={loadOrders}
            ...
            >
              ...
              >
                ...
                >
                  <span style={{ fontSize: '13px', color: '#333' }}>
                    Status:{' '}
                    {STATUS_MAP[selectedOrder.current_status]?.label ||
                ...
                >
                  {summaryLoading ? (
                    ...
                    >
                      ...
                      >
                        Click "Generate AI Summary" above to create
                        <br />
                        a comprehensive AI analysis of this order
                        <br />
      ...
      {shippingModal && (
        <div
          className="modal-overlay shipping-modal-overlay"
          onClick={closeShippingManager}
        >
          <div
          ...
          >
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={closeShippingManager}>
                ×
              </button>
            </div>

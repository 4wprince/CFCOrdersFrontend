 ...
 * v5.10.7 - Add X-Admin-Token to status and notes requests
 */
...
const OrderCard = ({ order, onOpenDetail, onOpenShippingManager, onUpdate }) => {
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = await res.text()
        throw new Error(msg || 'Failed to save notes')
      }
  ...
  const getShippedDaysRemaining = () => {
    const shippedShipments = (order.shipments || []).filter(
      (s) => s.status === 'shipped' && s.clock_started_at
    )
    ...
    const daysSince = Math.floor(
      (new Date() - latestClock) / (1000 * 60 * 60 * 24)
    )
    return Math.max(0, 5 - daysSince)
  }

  const daysRemaining = getShippedDaysRemaining()
  const alertBackground = order.alert_level
    ? ALERT_BACKGROUNDS[order.alert_level]
    : null
  const alertLabel = order.alert_type
    ? ALERT_TYPE_LABELS[order.alert_type]
    : null
    ...
    >
      {alertLabel && (
        <div
          style={{
            backgroundColor:
              order.alert_level === 'critical' ? '#d32f2f' : '#f57c00',
            color: 'white',
            padding: '4px 12px',
            fontSize: '12px',
            fontWeight: 'bold',
        ...
        <div className="order-financials">
          ...
          {shippingTotals.customerCharge > 0 && (
            <div className="shipping-summary">
              <span className="ship-charge">
                Ship: ${shippingTotals.customerCharge.toFixed(2)}
              </span>
              {shippingTotals.profit !== 0 && (
                <span
                  className={`ship-profit ${
                    shippingTotals.profit >= 0 ? 'positive' : 'negative'
                  }`}
                >
                  ({shippingTotals.profit >= 0 ? '+' : ''}
                  ${shippingTotals.profit.toFixed(2)})
                </span>
              )}
            </div>
          )}
        ...
        {warehouses.length > 0 && (
          <div className="warehouses">
            {warehouses.map((wh, i) => {
              ...
              const statusInfo =
                SHIPMENT_STATUS_COLORS[shipmentStatus] ||
                SHIPMENT_STATUS_COLORS.needs_order

              let clockDays = null
              if (shipment?.status === 'shipped' && shipment?.clock_started_at) {
                const clockDate = new Date(shipment.clock_started_at)
                const daysSince = Math.floor(
                  (new Date() - clockDate) / (1000 * 60 * 60 * 24)
                )
              ...
              return (
                <span
                  ...
                  title={
                    statusInfo.label +
                    (clockDays !== null ? ` (${clockDays} days to archive)` : '')
                  }
                  style={{
        ...
        {isEditingNotes ? (
          ...
          <div onClick={(e) => e.stopPropagation()}>
            {order.notes ? (
              ...
              >
                <div>
                  <strong style={{ color: '#7b1fa2' }}>Notes:</strong> {order.notes}
                </div>
                <button
